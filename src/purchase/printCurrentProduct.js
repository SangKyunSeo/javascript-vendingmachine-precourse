...
const title = [
    ...
    {name : '구매', class : 'purchase-button-header'}
]
...
export default class PrintCurrentProduct{
    ...
    // 구매하기 버튼 비활성화
    generatePossibleBuyProductButtonDisabled(text, name){
    ...
    generatePossibleBuyProductTableContent(){
        ...
        let productList = purchaseModel.getCurrentProductList('productList') === null ? 0 : purchaseModel.getCurrentProductList('productList');
        if(productList !== 0){
            for(let product of productList){
                ...
                if(product.quantity === 0){
                    row.appendChild(this.generatePossibleBuyProductButtonDisabled('구매하기','purchase-button'));    
                }else{
    ...
    generatePossibleBuyProduct(){
        ...
        const table = this.generatePossibleBuyProductTable('product-purchase-item-table');
